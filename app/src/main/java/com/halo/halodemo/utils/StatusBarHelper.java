...
//                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (DeviceHelper.isTablet(context) && sStatusbarHeight > DensityUtil.dp2px(STATUS_BAR_DEFAULT_HEIGHT_DP)) {
            //状态栏高度大于25dp的平板，状态栏通常在下方
            sStatusbarHeight = 0;
        } else {
            if (sStatusbarHeight <= 0 || sStatusbarHeight > DensityUtil.dp2px(STATUS_BAR_DEFAULT_HEIGHT_DP * 2)) {
                //安卓默认状态栏高度为25dp，如果获取的状态高度大于2倍25dp的话，这个数值可能有问题，用回桌面定义的值从新获取。出现这种可能性较低，只有小部分手机出现
                if (sVirtualDensity == -1) {
                    sStatusbarHeight = DensityUtil.dp2px(STATUS_BAR_DEFAULT_HEIGHT_DP);
                } else {
